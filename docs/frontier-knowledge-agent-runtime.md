...
前沿 Agent 系统的关键不只是更长上下文，而是上下文选择：按阶段检索相关知识、压缩历史、保留来源引用，并将工具权限和验收标准一并注入。Prompt 应版本化为能力模块，记录适用条件、失败恢复和回归案例；不要把一次成功的长 Prompt 当作稳定能力。

中文检索场景可在 lexical fallback 之上增加本地中文 Embedding、分词和 reranker adapter。模型是派生能力，不替代原始知识与证据账本；无模型时必须保留可用的词法回退。

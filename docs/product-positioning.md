...
### 6. 提示词与上下文工程｜Prompt & Context Engineering

灵犀把提示词当作可维护的能力配置，而不是一次性文本：岗位目标、输入输出、工具权限、失败恢复、事实/推断边界和验收标准都可以版本化为模块。运行时按阶段选择上下文，压缩无关历史，保留证据引用和未知项，让不同员工获得恰到好处的工作记忆。

### 7. 本地中文检索部署｜Local Chinese Retrieval

对于中文游戏发行资料，灵犀支持部署本地中文 Embedding、分词器和重排序模型作为检索适配器。模型、原文和索引可以留在用户设备；没有模型时，公开版使用 lexical fallback 保持可运行。这里的“支持部署”表示适配器和数据契约已定义，不代表仓库默认捆绑某个模型或硬件环境。
...
| Prompt / 上下文工程 | Prompt 扩展包、交付契约和阶段化运行链 | 可说“模块化 Prompt 与上下文控制已具备” |
| 本地中文检索 | 适配器设计 + lexical fallback | 可说“支持本地中文检索模型接入”，不说“默认内置模型” |
